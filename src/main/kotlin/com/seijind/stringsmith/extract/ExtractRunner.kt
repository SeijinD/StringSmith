...
object ExtractRunner {
    ...
    fun runQuick(project: Project, editor: Editor, file: PsiFile, settings: StringSmithSettings = StringSmithSettings.getInstance()) {
        ...
        val existingKey = StringsXmlUtil.findExistingKey(initialTarget, target.rawValue)
        val suggested = KeyGenerator.suggest(target.rawValue, settings.keyPrefix, settings.namingConvention, settings.maxKeyLength)
        val variants = StringsXmlUtil.findLocaleVariants(initialTarget)
        val collision = StringsXmlUtil.keyExists(initialTarget, suggested)

        val canSkip = allTargets.size == 1 &&
            existingKey == null &&
            !collision &&
            variants.isEmpty() &&
            KeyGenerator.isValidKey(suggested)

        if (!canSkip) {
            run(project, editor, file, settings)
            return
        }

        settings.lastTargetModulePath = initialTarget.path
        val result = ExtractDialogResult(
            key = suggested,
            reuseExisting = false,
            targetStringsXml = initialTarget,
            defaultValue = target.rawValue,
            localeEntries = emptyList()
        )
        ExtractWriter.writeExtract(project, editor, target, result, settings)
    }
